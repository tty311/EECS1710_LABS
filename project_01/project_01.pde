...
PImage bee,man,man1,man2,beeman;
PImage peach;
...
if(580< mouseX && mouseX< 800 && mouseY<410 && mouseY>280){
 ...
 PVector mousePos = new PVector(mouseX, mouseY);
  isRunning1 = position1.dist(mousePos) < 65;
  isRunning2 = position2.dist(mousePos) < 65;
  
  if (isRunning1) {
    position1 = position1.lerp(target1, 0.4);
    if (position1.dist(target1) < 65) {
      target1 = new PVector(random(0,500), random(410,570));
    }
  }
  image(man1,position1.x, position1.y, 40, 70);
  
  if (isRunning2) {
    position2 = position2.lerp(target2, 0.4);
    if (position2.dist(target2) < 65) {
      target2 = new PVector(random(200,800), random(410,570));
    }
  }
  image(man2,position2.x, position2.y, 40, 70);
